...
* 👥 **动态群成员鉴权与防拉群 (Dynamic Member Access Control)**：免去手动查询 Telegram 数字 ID 的麻烦，支持直接按 `@username` 授权；**只要是授权群内的成员，私聊 Bot 时自动完成动态成员身份核验**。检测到被拉入陌生未授权群聊时自动退出。
...
* 📖 **移动端即时预览 (Telegraph)**：长篇报告或方案支持一键转译为 Telegraph 页面，在 Telegram App 内支持 0 秒免密原生弹窗阅读。
