...
class TestConversationStateGate(unittest.IsolatedAsyncioTestCase):
    """Controller-owned state gate: L0's context-blind noise drop is vetoed
    when the bot spoke last (may be awaiting an answer) — the ack then
    reaches L2, which already judges bot-dialogue continuations. Layers stay
    context-blind; conversation state is the controller's responsibility."""
    ...
    def setUp(self):
        ...
        self.relay.broadcast_event = AsyncMock()
        self.ctrl = AutonomousController(
            bot_username="primary_bot",
            cfg=self.cfg,
            relay=self.relay,
    ...
    def _msg(self, text):
        return self.SimpleNamespace(
            chat_id=-1001234567890, chat_type="group", msg_id=300,
            sender_name="Alice", from_user={"is_bot": False}, text=text,
            is_bot_relay=False, attachments=None,
        )

    def test_bot_awaiting_answer_parsing(self):
        self.assertTrue(self.ctrl._bot_awaiting_answer("[Bot x]: 要开灯吗"))
        self.assertTrue(self.ctrl._bot_awaiting_answer("[Alice]: hi\n[Bot x]: q?"))
        self.assertFalse(self.ctrl._bot_awaiting_answer("[Alice]: 我走了"))
        self.assertFalse(self.ctrl._bot_awaiting_answer(""))
        self.assertFalse(self.ctrl._bot_awaiting_answer("[Alice]: a\n[b]: b"))

    async def test_state_gate_vetoes_noise_drop_when_bot_spoke_last(self):
        self.ctrl._context_summary_fn = lambda c, m, w: "[Alice]: 帮我开灯\n[Bot primary_bot]: 要开灯吗"
        classify = self.AsyncMock(return_value={
            "target_bot": "primary_bot", "target_bots": ["primary_bot"],
            "urgency": "immediate", "confidence": 0.9, "source": "classifier",
        })
        with __import__("unittest.mock", fromlist=["patch"]).patch.object(self.ctrl.arbiter, "classify", classify):
            await self.ctrl.evaluate_and_publish(self._msg("好的"))
        classify.assert_awaited_once()

    async def test_state_gate_closed_when_human_spoke_last(self):
        self.ctrl._context_summary_fn = lambda c, m, w: "[Bob]: 我下班了\n[Alice]: 好的"
        classify = self.AsyncMock(return_value={
            "target_bot": "primary_bot", "target_bots": ["primary_bot"],
            "urgency": "immediate", "confidence": 0.9, "source": "classifier",
        })
    ...
    async def test_engine_context_flattens_multiline_entries(self):
        import tempfile
        from groupconnect.core.config import GatewayConfig
        from groupconnect.core.context import ContextManager
        from groupconnect.engine import GroupConnectEngine
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            engine.context_mgr.record_message(
                -1001234567890, "Bot", "第一行\n第二行", msg_id=400, is_bot_reply=True,
                bot_username="guaguahome_bot",
            )
            ctx = engine._build_routing_context(-1001234567890, 0, 5)
            self.assertEqual(len(ctx.split("\n")), 1)
            self.assertIn("第一行 第二行", ctx)
