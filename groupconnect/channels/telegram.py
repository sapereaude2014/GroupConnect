...
from groupconnect.channels.extensions.geocoding import resolve_address
from groupconnect.channels.extensions.telegraph import process_outbound_text
    ...
    async def _process_update(self, update: Dict[str, Any]) -> None:
        ...
        if not raw_text:
            if "location" in msg:
                ...
                addr = await resolve_address(lat, lon, client=self.client)
                raw_text = (
                ...
                if not address:
                    address = await resolve_address(vloc.get("latitude"), vloc.get("longitude"), client=self.client)
